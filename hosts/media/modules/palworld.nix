...
in
{
  options.myServices.palworld.enable = lib.mkEnableOption "the Palworld dedicated server and its dashboard";

  config = lib.mkIf config.myServices.palworld.enable {
    ...
    virtualisation.oci-containers.containers = {
      ...
      palworld-dashboard = {
        ...
        dependsOn = [ "palworld" ];
        ports = [ "127.0.0.1:3939:3000" ]; # host 3939; loopback, internal via Caddy
        environment = {
